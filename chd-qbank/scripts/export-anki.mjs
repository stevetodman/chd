...
async function renderMedia(question, assets, usedFilenames) {
  ...
  const entries = [
    {
      ...
      kind: "cxr",
      render: (result) =>
        `<figure class="media image"><img src="${result.filename}" alt="${escapeHtml(result.alt)}" /></figure>`
    },
    {
      url: bundle.ekg_url,
      kind: "ekg",
    ...
    {
      url: bundle.diagram_url,
      kind: "diagram",
      render: (result) =>
        `<figure class="media image"><img src="${result.filename}" alt="${escapeHtml(result.alt)}" /></figure>`
    },
    {
      url: bundle.murmur_url,
      kind: "murmur",
      render: (result) => `<div class="media audio">[sound:${result.filename}]</div>`
    }
  ];
  ...
  for (const entry of entries) {
    ...
    const result = await handleMediaAsset(entry.url, question, entry.kind, bundle.alt_text, assets, usedFilenames);
    if (result) {
      fragments.push(entry.render(result));
    }
  }
