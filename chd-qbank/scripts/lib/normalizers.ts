...
export function normalizeItem(src: any, filePath: string, QuestionSchema: z.ZodTypeAny): NormResult {
  ...
  if (!choices) {
    for (const key of choiceKeyCandidates) if (out[key]) { choices = out[key]; break; }
  }
  if (!Array.isArray(choices)) { choices = []; warnings.push("no choices found -> created empty choices"); }

  // Ensure structure: id, label, text, isCorrect?
  const labelLetters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("");
  choices = choices.map((c: any, i: number) => {
    const cc = { ...c };
    if (!cc.id) cc.id = String(labelLetters[i] ?? `C${i+1}`);
    if (!cc.label) cc.label = labelLetters[i] ?? String(i+1);
    if (cc.text == null && cc.title != null) { cc.text = cc.title; }
    if (cc.text == null && typeof cc === "string") { return { id: cc.id, label: cc.label, text: c as string }; }
    return cc;
  });

  // Try to infer correct answer if legacy fields exist
  if (!choices.some((c: any) => c.isCorrect === true)) {
    let correctFrom: string | number | undefined;
    for (const k of answerKeyCandidates) {
      if (out[k] != null) { correctFrom = out[k]; break; }
    }
    if (typeof correctFrom === "number" && choices[correctFrom]) {
      ...
      const idxByLetter = labelLetters.indexOf(correctFrom.toUpperCase());
  ...
  // --- tags ---
  if (!Array.isArray(out.tags)) { out.tags = out.tags ? [String(out.tags)] : []; if (!out.tags.length) addedKeys.push("tags"); }

  // --- difficulty ---
  if (!out.difficulty) { out.difficulty = "med"; addedKeys.push("difficulty"); }
  else {
    const mapped = difficultyMap[String(out.difficulty).toLowerCase()];
    out.difficulty = mapped ?? "med";
  }

  // --- references / mediaBundle ---
  ...
  if (!Array.isArray(out.mediaBundle)) {
    ...
    out.mediaBundle = Array.isArray(legacyMedia) ? legacyMedia : (legacyMedia ? [legacyMedia] : []);
    if (out.media && !("media" in out.mediaBundle)) warnings.push("moved legacy 'media' into mediaBundle");
  }

  // --- offlineRequired ---
  if (typeof out.offlineRequired !== "boolean") { out.offlineRequired = false; addedKeys.push("offlineRequired"); }
