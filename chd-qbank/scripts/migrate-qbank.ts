...
// Import the same Zod schema used by the app
...
async function main() {
  ...
  if (files.length === 0) {
    console.log(pc.yellow("No JSON items found under content/questions."));
  }
  ...
  for (const file of files) {
    try {
      ...
      // Write normalized over original
  ...
  const body = rows.map(r =>
    [r.file, r.status, r.addedKeys, r.changedKeys, r.warnings, r.errors].map(escape).join(",")
  ).join("\n");
