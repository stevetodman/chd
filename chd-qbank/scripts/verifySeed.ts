...
async function verifyCounts(): Promise<void> {
  ...
  if ((questionCount ?? 0) < 10) fail(`Expected at least 10 questions, found ${questionCount ?? 0}`);
...
async function verifyQuestions(): Promise<void> {
  ...
  const { data, error } = await supabase
    .from("questions")
    .select("id,slug,status,media_bundle_id,correct_choice_id,context_panels,choices(id,label,is_correct)")
    .in("slug", slugs);
  ...
  for (const seed of QUESTIONS) {
    ...
    if (seed.context_panels && seed.context_panels.length > 0) {
      if (!row.context_panels || !Array.isArray(row.context_panels) || row.context_panels.length === 0) {
        fail(`Question ${seed.slug} missing context panels.`);
      }
    }

    if (!row.choices || row.choices.length !== seed.choices.length) {
      fail(`Question ${seed.slug} has ${row.choices?.length ?? 0} choices; expected ${seed.choices.length}.`);
    }
