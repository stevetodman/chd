...
type ResponseQueryResult = {
  data: ResponseRowResult[];
  error: Error | null;
};

type ResponseMaybeSingleResult = {
  data: ResponseRowResult | null;
  error: Error | null;
};
...
type ResponseQueryChain = {
  ...
  order: () => {
    limit: () => {
      maybeSingle: () => Promise<ResponseMaybeSingleResult>;
    };
  };
  then: (callback: (result: ResponseQueryResult) => void | Promise<void>) => Promise<void>;
};
          ...
          select: () => {
            ...
            const chain: ResponseQueryChain = {
              eq: (column: string, value: string) => {
                if (column === "user_id" || column === "question_id") {
              ...
              then: async (callback: (result: ResponseQueryResult) => void | Promise<void>) => {
                ...
                const mapped = matching.map((record) => mapRecord(record));
