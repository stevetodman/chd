...
export default function Dashboard() {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        ...
        for (const row of data ?? []) {
          const rawTopic =
            (row as { question?: { topic?: string | null } | null })?.question?.topic?.trim() ??
            'General practice';
        ...
        const insights: TopicInsight[] = Array.from(aggregates.entries()).map(
          ([topic, summary]) => ({
            topic,
            attempts: summary.attempts,
            correct: summary.correct,
            accuracy:
              summary.attempts > 0 ? Math.round((summary.correct / summary.attempts) * 100) : 0,
  ...
  const accuracy =
    metrics.total_attempts > 0
      ? Math.round((metrics.correct_attempts / metrics.total_attempts) * 100)
      : 0;
  ...
  const latestAccuracyPoint =
    typeof latestTrendPoint?.accuracy === 'number'
      ? Number(latestTrendPoint.accuracy.toFixed(1))
      : null;
  const previousAccuracyPoint =
    typeof previousTrendPoint?.accuracy === 'number'
    ...
    () => [
      {
        ...
        description:
          `${goalProgressPercent}% of your goal logged. ${attemptDeltaDescription} ${goalStatusMessage}`.trim(),
      },
      {
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      <section className="relative overflow-hidden rounded-3xl border border-brand-200 bg-gradient-to-r from-brand-600 via-brand-500 to-indigo-500 p-6 text-white shadow-sm md:col-span-2">
        <div
          className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(255,255,255,0.25)_0,transparent_55%)]"
          aria-hidden="true"
        />
      ...
      {aliasNeeded ? (
        <Card className="md:col-span-2" status="info">
          ...
          <CardContent className="space-y-3 text-sm text-sky-900">
            <p>
              Set your leaderboard alias in profile settings to participate. Aliases are visible to
              peers and locked after first save.
            </p>
      ...
      <Card className="md:col-span-2">
        <CardHeader>
          ...
          <p className="text-sm text-neutral-600">
            Real-time metrics from tutor mode and learning games.
          </p>
        </CardHeader>
            ...
            <>
              <section className="space-y-3 rounded-2xl border border-neutral-200 bg-neutral-50 p-4 text-sm text-neutral-700">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <p className="text-xs text-neutral-500">
                    Quick headlines for this week's progress.
                  </p>
                </div>
              ...
              <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <Card
                  ...
                  interactive
                  status="success"
                  className="p-0 text-emerald-900"
                >
                  <CardContent className="space-y-2">
                ...
                <Card elevation="flat" interactive status="info" className="p-0 text-sky-900">
                  <CardContent className="space-y-2">
                    ...
                    <p className="text-xs text-sky-700/80">
                      All-time total: {metrics.all_time_points}
                    </p>
                  </CardContent>
                </Card>
              </dl>
      ...
      <Card className="md:col-span-2 print:border print:border-transparent print:bg-white print:shadow-none">
        <CardHeader>
          ...
          <p className="text-sm text-neutral-600">
            Create an assistive summary to share with mentors or print for study check-ins.
          </p>
        </CardHeader>
        <CardContent className="space-y-4 text-sm text-neutral-700">
          ...
          <p className="text-xs text-neutral-500">
            This preview mirrors what is copied, shared, or printed so screen readers and
            collaborators receive the same context.
          </p>
        </CardContent>
      ...
      <Card className="md:col-span-2">
        <CardHeader>
          ...
          <p className="text-sm text-neutral-600">
            Adaptive goals and recommendations based on your recent activity.
          </p>
        </CardHeader>
        <CardContent>
          <div className="grid gap-4 lg:grid-cols-3">
            <section className="space-y-3 rounded-2xl border border-brand-100 bg-brand-50/60 p-4 text-sm text-brand-900">
              <div>
                ...
                <p className="text-xs text-brand-700/80">
                  {weeklyAttempts} of {weeklyGoalAttempts} practice attempts logged.
                </p>
              </div>
              <div
            ...
            <section className="space-y-3 rounded-2xl border border-amber-100 bg-amber-50/70 p-4 text-sm text-amber-900">
              ...
              <p>{milestoneNextMessage}</p>
              <p className="text-xs text-amber-700/70">
                Next celebration unlocks automatically once you cross the threshold.
              </p>
            </section>
            <section className="space-y-3 rounded-2xl border border-emerald-100 bg-emerald-50/70 p-4 text-sm text-emerald-900">
              <div>
              ...
              <p className="text-xs text-emerald-700/70">
                Combine high accuracy with steady practice to maximize exam confidence.
              </p>
            </section>
          </div>
          <div className="mt-6 grid gap-4 lg:grid-cols-2">
            ...
            <section className="space-y-3 rounded-2xl border border-neutral-200 bg-white p-4">
              ...
              {topicError ? (
                ...
                <p className="text-sm text-neutral-500">
                  Your accuracy is balanced. Rotate topics to stay sharp.
                </p>
              )}
            </section>
          </div>
          <div className="mt-6 flex flex-col gap-3 rounded-2xl border border-sky-100 bg-sky-50/70 p-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link
              to={cta.href}
              className="inline-flex items-center gap-2 self-start rounded-full bg-sky-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-sky-700"
            >
              {cta.label}
              <span aria-hidden="true">→</span>
            </Link>
      ...
      <Card>
        ...
        <CardContent>
          ...
          {loadingFeatured ? (
            <p className="mt-2 text-xs text-neutral-500">Loading fresh questions…</p>
          ) : null}
      ...
      <Card className="md:col-span-2">
        ...
        <CardContent className="grid gap-4 md:grid-cols-2">
          <Link
            to="/games/murmurs"
            className="rounded-lg border border-neutral-200 p-4 hover:border-brand-500"
          >
            Guess the Murmur →
          </Link>
          <Link
