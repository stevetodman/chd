...
export default function Login() {
  ...
  const handleSubmitPasswordReset = async (event: FormEvent) => {
    ...
    try {
      ...
      setResetError(
        getErrorMessage(
          err,
          t('auth.login.resetErrorFallback', {
            defaultValue: 'Unable to send password reset email',
          }),
        ),
      );
    } finally {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      setError(
        getErrorMessage(err, t('auth.login.signInError', { defaultValue: 'Unable to sign in' })),
      );
    } finally {
      setSigningIn(false);
    }
