...
describe('CXR bounding box hit testing', () => {
  ...
  it('keeps fractional display pixels inside the tolerance band', () => {
    ...
    expect(hitTestDisplayPoint(roundedDisplay, bbox, naturalSize, displaySize, tolerance)).toBe(
      true,
    );
  });
  ...
  it('correctly maps points recorded in device pixels when DPR > 1', () => {
    ...
    expect(hitTestDisplayPoint(devicePixelPoint, lesion, natural, display, tolerance, dpr)).toBe(
      true,
    );
  });
});
