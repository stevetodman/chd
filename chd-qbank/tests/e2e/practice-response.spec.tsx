...
const practiceQuestion: QuestionQueryRow = {
  ...
  choices: [
    { id: 'choice-a', label: 'A', text_md: 'Order a Holter monitor', is_correct: false },
    {
      id: 'choice-b',
      label: 'B',
      text_md: 'Schedule transthoracic echocardiography',
      is_correct: true,
    },
  ],
};
      ...
      eq: (_column: string, value: string) => ({
        select: () => ({
          maybeSingle: async () => {
            ...
            if (payload.ms_to_answer !== undefined)
              record.ms_to_answer = payload.ms_to_answer ?? null;
...
function computeReliability(state: SupabaseMockState): ReliabilitySnapshot {
  ...
  const totalAttempts = Array.from(itemStats.values()).reduce(
    (acc, value) => acc + value.attempts,
    0,
  );
...
describe('practice response analytics capture', () => {
  beforeEach(() => {
    ...
    useSessionStore.setState({
      session: createMockSession(TEST_USER_ID),
      loading: false,
      initialized: true,
    });
  });
